...
namespace Endpoint_EightToTen.Services.Mini9B
{
    public class Mini9BService : IMini9BService
    {
        
        public string GuessItNormal (string input)
        {
            Random rng = new Random();
            int randNum = rng.Next(1, 50);
            int parsedInput;
            if (int.TryParse(input, out parsedInput))
            {
                if (parsedInput > 50 || parsedInput < 1)
                {
                    return "Your guess needs to be between 1 and 50. Please try again.";
                }
